...
public class BoidBehavior : MonoBehaviour
{
    ...
    [SerializeField] float separationModifier;
    [SerializeField] float alignmentModifier;
    [SerializeField] float cohesionModifier;
    ...
    Vector3 BoidIntereaction()
    {
        ...
        Collider[] neighboringBoids = Physics.OverlapSphere(gameObject.transform.position, detectionDistance, LayerMask.GetMask("Obstacle"));
        ...
        else
        {
            ...
            foreach (Collider neighborBoid in neighboringBoids)
            {
                ...
                separationVector += neighborAvoidanceVector * separationModifier;
                ...
                alignmentVector += neighborMovementVector.normalized * alignmentModifier;
                ...
                cohesionVector += ((neighborPosition - gameObject.transform.position) / neighboringBoids.Length) * cohesionModifier;
...
//      TODO: check if raycasts should be done on FixedUpdate()
        ...
        for (int i = 0; i < rayCount; ++i)
        {
            ...
            // strength is based on inverse square law ('normalized = vector / magnitude', so only need 1 more "/ magnitude")
            if (Physics.Raycast(transform.position, Quaternion.AngleAxis(rayAngle, transform.up) * transform.forward, out hit, detectionDistance, LayerMask.GetMask("Boid")))
            {
